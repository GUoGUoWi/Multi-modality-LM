...
class COCOTextImageDataset(Dataset):
    ...
    def _get_generated_caption_path(self,item):
        ...
        # 确定正确的子目录
        if client_id is not None:
            generated_captions_dir = os.path.join(self.generated_captions_dir, f"client_{client_id}")
            os.makedirs(generated_captions_dir, exist_ok=True)
        else:
    ...
    def __getitem__(self, idx):
        ...
        is_generated_image = item.get("is_generated_image")
            ...
            if gen_path:
                try:
                    ...
                    if has_image:
                        ...
                        item["is_generated_image"] = is_generated_image
                except Exception as e:
...
class DataProcessor:
    ...
    def get_dataloader(self, data_list, batch_size=Config.BATCH_SIZE, is_train=True):
        ...
        def custom_collate_fn(batch):
            ...
            # 确保is_generated_image是tensor
            is_generated_image = [
                item.get("is_generated_image", torch.tensor(False)) if isinstance(item.get("is_generated_image", False),
                                                                            torch.Tensor)
                else torch.tensor(item.get("is_generated_image", False), dtype=torch.bool) for item in valid_batch]
