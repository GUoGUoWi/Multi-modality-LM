...
def main():
    ...
    logger.info("Baseline 5: 引入主动方（LLaVA-7B）的垂直联邦学习， 并使用BLIP为非对齐缺失描述的模型训练")
    ...
    # 为客户端生成缺失描述
    aggregation = True      # baseline5 对齐数据使用聚合后大模型
    generate_missing_captions(client_data, aggregation=aggregation)
    
    generate_missing_captions(client_data, aggregation=False) #各方非对齐数据补全

    # 从原始数据中为服务端准备数据
    logger.info("为客户端准备数据...")
