...
def federated_round(server, clients, processor, round_num, logger):
    ...
    # # 服务端使用相同的对齐数据（这里使用最后客户端的对齐数据，因为内容一致）
...
def main():
    ...
    logger.info("Baseline 3: 引入主动方（LLaVA-7B）的垂直联邦学习")
    ...
    for client_id in range(Config.NUM_CLIENTS+1):
        ...
        logger.info(f"Client {client_id}: {model_type.name}")
        # 获取数据加载器
