...
class COCOTextImageDataset(Dataset):
    ...
    def _get_generated_image_path(self, item):
        ...
        if self.aggregation:
            # 确定正确的子目录
            generated_images_dir = os.path.join(self.generated_images_dir, "aggregation")
    ...
    def __getitem__(self, idx):
        ...
        if "image_id" in item and item["image_id"] is not None and not self.text_only:
            has_image = True
            img_path = os.path.join(self.image_dir, f"{item['image_id']:012d}.jpg")
        ...
        # 尝试使用生成的图像替代缺失的图像
        if not has_image and self.use_generated_images:
            gen_path = self._get_generated_image_path(item)
            if gen_path:
                try:
                    ...
                    print(f"Error loading generated image {gen_path}: {e}")
        ...
        sample = {
            ...
            "has_caption": torch.tensor(True, dtype=torch.bool),
            "is_generated_image": torch.tensor(is_generated_image, dtype=torch.bool)
        }
...
class DataProcessor:
    ...
    def get_dataloader(self, data_list, batch_size=Config.BATCH_SIZE, is_train=True):
        ...
        def custom_collate_fn(batch):
            ...
            # 确保has_image是tensor
            has_images = [
                item.get("has_image", torch.tensor(False)) if isinstance(item.get("has_image", False), torch.Tensor)
                else torch.tensor(item.get("has_image", False), dtype=torch.bool) for item in valid_batch]
            has_images = torch.stack(has_images)

            # 确保is_generated是tensor
            is_generated_image = [
                item.get("is_generated_image", torch.tensor(False)) if isinstance(item.get("is_generated", False),
                                                                            torch.Tensor)
                else torch.tensor(item.get("is_generated", False), dtype=torch.bool) for item in valid_batch]
            is_generated_image = torch.stack(is_generated_image)
            ...
            return {
                ...
                "is_generated_image": is_generated_image,
                "image_id": image_ids
            }
    ...
    def split_data_for_clients(self, data, num_clients=Config.NUM_CLIENTS):
        ...
        # 处理训练集对齐数据 - 每个客户端都有相同的对齐图像，但不同的描述
        for img_id in data["train"]["aligned_images"]:
            ...
            for client_id in range(min(num_clients, len(captions))):
                # 根据对齐数据图像缺失比例决定是否包含图像
                has_image = random.random() > Config.ALIGNED_IMAGE_MISSING_RATIO

                # 创建样本
                sample = {
                    'image_id': img_id if has_image else None,
                    'caption': captions[client_id],
                    'category_id': category_id,
                    'client_id': client_id  # 添加客户端ID以便生成图像时知道来源
                }
        ...
        if len(data["train"]["non_aligned_images"]) < total_non_aligned_needed:
            ...
            for client_id in range(num_clients):
                ...
                for sample in client_samples:
                    if random.random() < Config.NON_ALIGNED_IMAGE_MISSING_RATIO:
            ...
            for client_id in range(num_clients):
                ...
                for img_id in client_non_aligned_images:
                    ...
                    # 根据非对齐数据图像缺失比例决定是否包含图像
                    has_image = random.random() > Config.NON_ALIGNED_IMAGE_MISSING_RATIO
    ...
    def get_sim_matrix(self, prototype=None, client_data=None, for_server=False, zero_aligned = False):
        ...
        if zero_aligned:
            ...
            prototype_tensor = torch.cat([data[0].unsqueeze(0) for i, data in prototype.items()], dim=0)
    ...
    def append_aligned_data(self,aligned_pairs, prototypes=None, client_features = None, server_features = None, attention_mask = None):
        self.logger.info("正在使用伪对齐数据补充对齐数据")
        selected_aligned_pairs = []
        if attention_mask:
            for i, j, k in aligned_pairs:
                if (i, j, k) in attention_mask:
                    continue
                selected_aligned_pairs.append((i,j,k))
        else:
            attention_mask = []
